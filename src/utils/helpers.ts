...
const arbitrarySupportedClasses: any = {
  pt: "padding-top",
  pb: "padding-bottom",
  pl: "padding-left",
  pr: "padding-right",
...
const convertToCss = (classNames: string[]) => {
  ...
  const arbitraryClasses: any = classNames.filter((className) =>
    className.includes("[")
  );

  arbitraryClasses.forEach((className: any) => {
...
export const getConvertedClasses = (input: any) => {
  const classNames = input.split(/\s+/).map((i: any) => "." + i.trim());
